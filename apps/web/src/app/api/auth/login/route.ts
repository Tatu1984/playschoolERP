...
import { clearRateLimit, clientIp, enforceRateLimit, LOGIN_LIMIT } from "@/backend/utils/rate-limit.util";
...
export async function POST(req: NextRequest) {
  try {
    ...
    await enforceRateLimit(LOGIN_LIMIT, `ip:${clientIp(req)}`);
    await enforceRateLimit(LOGIN_LIMIT, `email:${email}`);
    ...
    await clearRateLimit(LOGIN_LIMIT, `email:${email}`);
